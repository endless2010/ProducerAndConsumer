public class Producer implements Runnable{
	...
	public Producer (MainFrame frame,String name,Semaphore mutex,Semaphore full,Semaphore empty,Buffer buf){
		this.mutex=mutex;
		this.full=full;
		this.empty=empty;
		this.buf=buf;
	...
	public void run(){
		while(true){
			empty.p();
			mutex.p();
			frame.jta.append("Producer "+name+" insert a new product into "+buf.number+"\n");
			timesOfProducer++;
			buf.value++;
			if(buf.number==1){
